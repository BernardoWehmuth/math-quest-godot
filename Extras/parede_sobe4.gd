...
func _ready() -> void:
	SignalConcluido.connect("concluido", Callable(self, "move_platform"))
...
func move_platform():
	if Difficulty.dificuldade == 2:
		...
		var platform_tween = create_tween().set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
